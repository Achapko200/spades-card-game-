...
Please enter a number between 1 and 1 inclusive:\n\
"};
		Card one = player->play_card(Suit::HEARTS);
		Card two = player->play_card(Suit::HEARTS);
		Card three = player->play_card(Suit::CLUBS);
		Card four = player->play_card(Suit::HEARTS);
		Card five = player->play_card(Suit::CLUBS);
		Card six = player->play_card(Suit::DIAMONDS);

		player->add_card(card_ac);
		player->add_card(card_4h);
		Card seven = player->play_card(Suit::SPADES);
		Card eight = player->play_card(Suit::SPADES);

		REQUIRE(oss.str() == expected_output);
		REQUIRE(one == card_4h); // 1
		REQUIRE(two == card_ac); // 1
		REQUIRE(three == card_8d); // 2
		REQUIRE(four == card_js); // 3
		REQUIRE(five == card_7d); // 1
		REQUIRE(six == card_2s); // 1
		REQUIRE(seven == card_4h); // 2
		REQUIRE(eight == card_ac); // 1
